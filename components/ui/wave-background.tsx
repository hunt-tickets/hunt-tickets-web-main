...
export function WaveBackground({ className, style, extendLeftPx = 320 }: WaveBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const FilmGrainShader = {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          noise = noise * 2.0 - 1.0;
          vec3 result = color.rgb + noise * intensity * 0.05;
          gl_FragColor = vec4(result, color.a);
        }
      `,
    };

    function createFilmGrainPass(intensity = 0.6, grainScale = 0.3) {
    ...
    waveScene.add(new THREE.AmbientLight(0xffffff, 0.1));
    ...
    function createInstancedMaterial() {
      // Monochromatic colors - using grays/whites
      const baseCol = new THREE.Color(0.9, 0.9, 0.9); // light gray
      const emisCol = new THREE.Color(1.0, 1.0, 1.0); // white
      ...
      return new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main(){
            ...
            float emissiveStrength = uBaseEmissive + vGlow * 0.6 + vPulse * 0.1;
            vec3 finalColor = uColor + uEmissive * emissiveStrength;
            gl_FragColor = vec4(finalColor, 0.2 * alpha);
          }
        `,
    ...
    function initWaveThree() {
      ...
      waveBloomPass = new UnrealBloomPass(new THREE.Vector2(cameraWidth, cameraHeight), 0.8, 0.5, 0.0);
